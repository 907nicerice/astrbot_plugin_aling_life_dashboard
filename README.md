...
顶部“正式 / 测试”按钮用于切换数据视图。正式视图保持原有模块；测试视图只显示带 `test-account:` 命名空间的数据，并隐藏 QQ 空间入口和面板。

测试视图提供带二次确认的“清空测试记忆”操作，只删除 `test-account:` 命名空间下的长期记忆、摘要、短期余波和 Companion 状态，不触碰正式数据。
